...
def get_validation_idxs(df, len_seq_in, len_seq_out, data_split, region): 
    ...
    for i in range(df.shape[0]):
        if df.iloc[i]['split_type'] == split_type:
            input_output_seq = get_io_validation_times(df, i, len_seq_in, len_seq_out, region)
            if input_output_seq: 
...
def get_sequence(seq, root, data_split, region, product, bands, preprocess=None, swap_time_ch=False, ds=None):
    ...
    mask_seq = []
    for s in seq:
        prods, masks = get_file(s, root, data_split, region, product, bands, preprocess, ds)
...
def get_file(sample_id, root, data_split, region, product, bands, preprocess=None, ds=None):
    ...
    if product == 'RATE': 
        for b in ['rainfall_rate-500X500']:
            ...
            if VERBOSE: print(time.time()-file_t,"OPERA preprocess time")
    else: 
        for j, b in enumerate(bands):
            if preprocess is not None:
                x[j] = preprocess_HRIT(x[j], preprocess[b])
            if VERBOSE: print(time.time()-file_t,"HRIT preprocess time")

    #return format - channels x width x height
    if VERBOSE: print(time.time()-file_t,"Total File Read Time")
    return x, masks
...
def preprocess_fn(x, preprocess, verbose=False):
    ...
    if verbose: print(0, np.unique(x))
    # 1 Map values
...
def load_timestamps(path, types={'time': 'str', 'date_str': 'str', 'split_type': 'str'}):
...
def crop_xarray(product, x_start, y_start, size):
    ...
    """    
    return product.isel(x=slice(x_start, x_start+size), y=slice(y_start, y_start+size))
...
def prepare_crop(location, opera_k=1):    
    ...
    # scale if opera
    if opera_k!=1: context, roi, position = [opera_k*s for s in [context, roi, position]]
    context_size = context+roi+context
    
    context_shape = {'x_start': position[0], 'y_start': position[1], 'size': context_size}
...
def tensor_to_submission_file(predictions, predict_params):
    ...
    submission_path = os.path.join('submission',str(predict_params['year_to_predict']),submission_file_name)   
